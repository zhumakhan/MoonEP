'''
Run:
    torchrun --nproc-per-node=8 my_moon_moe.py
'''
...
class _MoonEpDispatch(torch.autograd.Function):
    ...
    1 <= s <= S, into the expert-grouped shard accross ranks. The returned
    ...
    the slot gradients back: each rank's K slot grads are summed into
    grad_x, and each each slot's weight grad is gathered back to its (token, K)
    positions. combine accepts [nvs_s, ...] grads directly
    """
    ...
    def forward(
        ...
        topk: torch.Tensor, # [s, K], token to its topk expert mapping. expert_idx = topk[token_idx]
        tpe: torch.Tensor, # [E], local histogram of token distribution over experts
        buffer: Buffer, # MoonEP's buffer object
        holder: Any # object for bookkeeping
    ):
...
class _MoonEPCombine(torch.autograd.Function):
    ...
    re-dispatches the output grad with the save plan: grad_out[t] is
...
class MoonEPMoe(nn.Module):
    ...
    Mixed prevision: fp32 masters (``parameters()``) for the optimizer, bf16
    compute copies of the experts for the forward/backward, fp32 gating. The
    expert masters' ``.grad`` alies the module's persistent fp32 grad storage,
    ...
    def __init__(self, E, K, H, Hi, S, group=None, num_sms=32):
        ...
        # Its grad is summed over the EP group in redice_expert_grad().
        torch.manual_seed(0)
    ...
    # the same node, of a uint8[64] CPU fabric handle. Every rank calls these
    ...
    def _grad_buffers(self, chunk_shape, group):
        ...
        try:
            ...
            self._close_items(
                g_items + r_items + [r_local, self._local_item(g_share, use_fabric)], use_fabric
            )
    ...
    def _register_grad_fold(self, w16, targets):
        ...
        are folded: rows of ther ranks' experts are live remote mappings
    ...
    def zero_grad(self, set_to_none: bool = True):
        ...
        already None after the fold hooks; reduce_grad clear the consumed
        reduce rows itself.
        """
        with torch.no_grad():
            for mg in (self.mg_gate, self.mg_up, self.mg_down):
    ...
    def sync_compute_weights(self):
        """fp32 experts masters -> bf16 compute copies (the router has no bf16
        copy). Call after every optimizer step. Only thi ranks's own experts
    ...
    def forward(self, x):
        ...
        capacity: the comm kernels loop over s, no pading tokens are made.
        """
    ...
    def reduce_expert_grads(self):
        ...
        # tokens, matching the summed expert grads) so every replicate takes the
        ...
        # reduce_grad has not entry barrier: every rank's backward (the grad
        # hooks) must have finished writing its reduce chunk before any rank
        # remote-reads it. Device-side sync, not host round trip
    ...
    def forward_reference(self, x):
        """Single rank reference: per-expert loop over the global weight ppol
...
def reference_grads(moe, x, gout):
    ...
    return out, xr.grad, rw.grad, wg.grad, wu.grad, wg.grad
...
def main():
    ...
    # skeq the routing by zeroing part of the (fp32, replicated) router
    with torch.no_grad():
        moe.router_w[:64].zero_()
        
    # AdamW over the fp32 masters only: parameters() excludes the bg16 compute
    # copies. The masters' .grad alias MoonEPMoe's persistent fp32 grad
    # stoage, so grads are cleared with moe.zero_grad(), never opt.zero_grad().
    ...
    # masters, and the replicated router must be but-identical on every rank
